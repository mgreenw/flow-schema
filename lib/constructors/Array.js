...
class ArraySchema<T> extends Schema<T[]> {
  ...
  min(length: number) {
    assert(length >= 0, `Minimum array length constraint must be positive (received ${length})`);
  ...
  max(length: number) {
    assert(length >= 0, `Maximum array length constraint must be positive (received ${length})`);
    return this.constrain({
      description: `length < ${length}`,
      constrain: array => (array.length < length ? array : undefined),
    });
