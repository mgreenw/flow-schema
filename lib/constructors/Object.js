...
import { ensureObject, type AnyObject } from '../utils/object';
...
function object<Obj: AnyObject>(propertySchemas: Obj): Schema<$ObjMap<Obj, SchemaType>> {
  return new Schema({
    ...
    validate: value => {
      ...
      return Object.keys(propertySchemas).reduce((acc, key) => {
        acc[key] = propertySchemas[key].validate(obj[key], { propertyName: key });
        return acc;
      }, {});
    },
  });
